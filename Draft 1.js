...
// Spotify Web Player Stuff 
public class SpotifyWebPlayer {
    public static void main(String[] args) throws Exception {
        // Spotify API credentials
        String CLIENT_ID = "27d4f492e1854ed5b3fec033f1a26fdd'";
        String CLIENT_SECRET = "25823a0399864b2e86adc643a7fc1b43";
        ...
        conn.setRequestProperty("Authorization", "Basic " + Base64.getEncoder().encodeToString(('27d4f492e1854ed5b3fec033f1a26fdd' + ":" + '25823a0399864b2e86adc643a7fc1b43')).getBytes()));
        conn.setDoOutput(true);
        DataOutputStream os = new DataOutputStream(conn.getOutputStream());
        os.writeBytes("grant_type=client_credentials");
        os.flush();
        os.close();
        ...
        if (responseCode == HttpURLConnection.HTTP_OK) {
            ...
            URL playlistUrl = new URL("https://api.spotify.com/v1/playlists/7DleqSRA4IChb6px84CcMc?si=4a7b405d2e9942ca");
            HttpURLConnection playlistConn = (HttpURLConnection) playlistUrl.openConnection();
            playlistConn.setRequestMethod("GET");
            playlistConn.setRequestProperty("Authorization", "Bearer " + accessToken);

            int playlistResponseCode = playlistConn.getResponseCode();
            if (playlistResponseCode == HttpURLConnection.HTTP_OK) {
                BufferedReader playlistIn = new BufferedReader(new InputStreamReader(playlistConn.getInputStream()));
                String playlistInputLine;
                StringBuffer playlistResponse = new StringBuffer();
                while ((playlistInputLine = playlistIn.readLine()) != null) {
                    playlistResponse.append(playlistInputLine);
                }
                playlistIn.close();

                // Parse playlist tracks
                JSONObject playlistJsonResponse = new JSONObject(playlistResponse.toString());
                JSONArray tracks = playlistJsonResponse.getJSONObject("tracks").getJSONArray("items");

                // Select a random track
                int randomIndex = (int) (Math.random() * tracks.length());
                String randomTrackUri = tracks.getJSONObject(randomIndex).getJSONObject("track").getString("uri");

                // Embed Spotify Web Player
                System.out.println("Embedding Spotify Web Player with random track: " + randomTrackUri);
                // Use the retrieved track URI to embed the Spotify Web Player on your website
            } else {
                System.out.println("Failed to retrieve playlist information. Response code: " + playlistResponseCode);
            }
        } else {
            System.out.println("Failed to authenticate with Spotify API. Response code: " + responseCode);
        }
    }
}
